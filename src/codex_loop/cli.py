...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            print( "  3. Trust this project in Codex (~/.codex/config.toml):")
            print( "     codex-loop run runs Codex inside an isolated Git worktree — both paths below must be trusted.")
            print( "     Add these two entries to ~/.codex/config.toml (create it if it doesn't exist):")
            print( "       mkdir -p ~/.codex")
            print( "       open ~/.codex/config.toml   # macOS — or: nano ~/.codex/config.toml  (Linux)")
            print( "     Paste:")
            print(f"         [projects.\"{project_dir}\"]")
            print( "         trust_level = \"trusted\"")
            print()
            print(f"         [projects.\"{_wt_parent}\"]")
            print( "         trust_level = \"trusted\"")
            print( "     Save the file, then verify both entries are there:")
            print( "       cat ~/.codex/config.toml")
            print( "     (Without this step, codex-loop run fails with 'Not inside a trusted directory'.)")
