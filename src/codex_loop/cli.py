...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            print()
            print("Next steps:")
            print( "  1. Open codex-loop.yaml and confirm verification.commands runs your tests. For example:")
            print( "       Python:  \"python -m pytest tests/ -q\"")
            print( "       Node:    \"npm test\"")
            print( "       Go:      \"go test ./...\"")
            print( "     (codex-loop.yaml uses JSON syntax, not YAML indentation — that is normal.)")
            print( "     Tip: run the verification command manually now to confirm it works before starting the loop.")
            print( "  2. Skim tasks/ to confirm the tasks match your goal.")
