...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            print( "       ls tasks/                     # task list (aim for 2-5 tasks)")
            print( "       cat spec/001-project-spec.md  # full spec")
            print( "     Too many tasks? Delete the ones you don't need, or re-run with a narrower prompt.")
            print( "     Tasks too vague? Edit them to be more specific before running.")
            print( "     If the output looks wrong, re-run: codex-loop init --prompt \"...\" --force")
            ...
            print( "     After editing, verify it is valid JSON: python3 -m json.tool codex-loop.yaml")
            ...
            _wt_parent = project_dir.parent / '.codex-loop-worktrees'
            print( "  3. Trust the worktree path in Codex (~/.codex/config.toml):")
            print( "     Since init succeeded, your project directory is already trusted.")
            print( "     codex-loop run runs Codex in an isolated worktree next to your project — that path also needs trust.")
            print( "     Open ~/.codex/config.toml and make sure this entry is present (add it if missing):")
            print( "       mkdir -p ~/.codex")
            print( "       open ~/.codex/config.toml   # macOS — or: nano ~/.codex/config.toml  (Linux)")
            print(f"         [projects.\"{_wt_parent}\"]")
            print( "         trust_level = \"trusted\"")
            print( "     (The entry for your project directory should already be there from when you ran init.)")
            print( "     Verify both are present:")
            print( "       cat ~/.codex/config.toml")
            print( "     (Without the worktree entry, codex-loop run fails with 'Not inside a trusted directory'.)")
            print( "  4. Commit the generated files so Codex can see them in its isolated worktree:")
