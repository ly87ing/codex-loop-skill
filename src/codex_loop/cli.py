...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            print( "     codex-loop run runs Codex in an isolated worktree — that path also needs trust.")
            print( "     Run this command to add the missing entry (safe to run more than once — skips if already present):")
            print(f"       mkdir -p ~/.codex && grep -qF '[projects.\"{_wt_parent}\"]' ~/.codex/config.toml 2>/dev/null || printf '\\n[projects.\"{_wt_parent}\"]\\ntrust_level = \"trusted\"\\n' >> ~/.codex/config.toml")
            print( "     Verify both entries are present:")
