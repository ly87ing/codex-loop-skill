...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            print()
            print("Next steps:")
            print( "  1. Open codex-loop.yaml — it uses JSON syntax (not YAML indentation), that is normal.")
            print( "     Check that verification.commands matches how you run your tests. For example:")
            print( "       Python:  \"python -m pytest tests/ -q\"")
            print( "       Node:    \"npm test\"")
            print( "       Go:      \"go test ./...\"")
            print( "  2. Skim tasks/ to make sure the tasks look right.")
            ...
            print( "       git init && git add -A -- ':!.codex-loop' && git commit -m 'init'")
            print( "       (skip if you already have commits; avoid 'git add .' — it would include .codex-loop/)")
