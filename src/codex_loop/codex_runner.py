        ...
        if completed.returncode != 0:
            ...
            if "not inside a trusted" in combined or "trusted directory" in combined:
                ...
                hint = (
                    ...
                    "If this happened during 'codex-loop init', re-run with --model:\n"
                    "  codex-loop init --prompt \"...\" --model o3\n"
                    "If this happened during 'codex-loop run', edit codex-loop.yaml:\n"
                    '  "codex": { "model": "o3" }\n'
                    "Then re-run."
                )
            ...
            elif "unknown command" in combined or ("exec" in combined and ("unknown" in combined or "unrecognized" in combined or "invalid" in combined)):
                hint = (
                    "\nHint: your Codex CLI does not support the 'exec' subcommand. "
                    "Update Codex CLI to the latest version:\n"
                    "  npm install -g @openai/codex\n"
                    "  # or: brew upgrade --cask codex"
                )
