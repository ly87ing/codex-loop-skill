...
- `snapshots` reads that directory-level `index.json` back as an operator view, with task filtering, status filtering, `--latest`, ISO time windows via `--since/--until`, raw JSON output, and a `--summary` aggregation over task, status, selection, blocker code, and latest snapshot markers.
