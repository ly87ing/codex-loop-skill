...
- `codex-loop doctor --repair`:
  ...
  - warns when cleanup defaults are aggressive enough to delete all retained artifacts on apply
...
- `events --summary` aggregates the filtered event set by label, task, source, blocker code, blocked task, and latest blocked event before optional JSON/export handling.
...
- `doctor` also warns when `operator.cleanup` defaults are configured with `keep=0` and no age threshold, which would make `cleanup --apply` destructive by default.
