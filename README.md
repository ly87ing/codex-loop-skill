...
- `snapshots` reads that directory-level `index.json` back as an operator view, with task filtering, `--latest`, raw JSON output, and a `--summary` aggregation over task, status, selection, blocker code, and latest snapshot markers.
